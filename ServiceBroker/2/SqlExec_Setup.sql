...
CREATE TABLE Logging.LogEntry (
    ...
    ,LogEntrySeriesId uniqueidentifier NOT NULL DEFAULT(NEWID())
...
CREATE OR ALTER PROCEDURE SqlExec.Process_SqlExec_ProcessingQueue
AS
SET NOCOUNT ON;
SET ARITHABORT ON;
...
BEGIN
    ...
    BEGIN TRY
        ...
        (
            ...
            FROM SqlExec.SqlExec_ProcessingQueue
        )
...
IF EXISTS (SELECT 1 FROM sys.service_queues WHERE name = 'SqlExec_InitQueue' AND SCHEMA_NAME(schema_id) = 'SqlExec')
    DROP QUEUE [SqlExec].[SqlExec_InitQueue]
GO

IF EXISTS (SELECT 1 FROM sys.service_queues WHERE name = 'SqlExec_ProcessingQueue' AND SCHEMA_NAME(schema_id) = 'SqlExec')
    DROP QUEUE [SqlExec].[SqlExec_ProcessingQueue]
GO
...
CREATE QUEUE SqlExec.SqlExec_InitQueue
    WITH STATUS=ON
    ,ACTIVATION (
        MAX_QUEUE_READERS = 8
        ,EXECUTE AS SELF
        ,PROCEDURE_NAME = SqlExec.Process_SqlExec_ProcessingQueue
    )
GO

CREATE QUEUE SqlExec.SqlExec_ProcessingQueue
    WITH STATUS = ON
    -- ,ACTIVATION (
    --     MAX_QUEUE_READERS = 8
    --     ,EXECUTE AS SELF
...
CREATE SERVICE [//SqlExec/ProcessingService] AUTHORIZATION dbo
    ON QUEUE SqlExec.SqlExec_ProcessingQueue
GO

CREATE SERVICE [//SqlExec/InitService] AUTHORIZATION dbo
    ON QUEUE [SqlExec].[SqlExec_InitQueue]
GO
...
BEGIN DIALOG CONVERSATION @conversationHandleId
    ...
    TO SERVICE '//SqlExec/ProcessingService', 'CURRENT_DATABASE'
...
SELECT * FROM SqlExec.SqlExec_ProcessingQueue
SELECT * FROM SqlExec.SqlExec_InitQueue
