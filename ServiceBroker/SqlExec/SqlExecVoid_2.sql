...
CREATE TABLE Logging.LogEntry (
    ...
    ,CallingObject nvarchar(256) NULL DEFAULT(ISNULL(ERROR_PROCEDURE(),OBJECT_NAME(@@PROCID)))
...
BEGIN
    IF @_isVerbose = 1
        RAISERROR(N'Committing back transactions',0,0) WITH NOWAIT;
...
BEGIN
    ...
    BEGIN
        ...
        SET @logEntrySeriesId = NEWID()



        -- Get details JSON
        SELECT
            @detailsJson = @messageBody.value('(/request/details/text())[1]', 'nvarchar(max)')

        SET @detailsJson = TRIM(@detailsJson)

        SELECT
            @batchId = JSON_VALUE(@detailsJson, '$.details[0].batchId')
            ,@sql = @messageBody.value('(/request/sql/text())[1]', 'nvarchar(max)')
...
CREATE OR ALTER PROCEDURE dbo.ExecuteSqlAsyncVoid(
    @sql nvarchar(max)
    ,@batchId UNIQUEIDENTIFIER
    ,@conversationGroupId UNIQUEIDENTIFIER
)
...
SELECT StartTime = @startTime
    ...
    ,TotalMin = DATEDIFF(m, @startTime, @endTime)
