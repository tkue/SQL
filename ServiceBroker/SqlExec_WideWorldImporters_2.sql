...
-- EXEC master..uspKillDatabaseConnections 'WideWorldImporters'
-- GO

-- -- restore filelistonly from disk = '/var/opt/mssql/data/WideWorldImportersFull.bak'
-- RESTORE DATABASE WideWorldImporters
-- FROM DISK = '/var/opt/mssql/data/WideWorldImportersFull.bak'
-- WITH STATS=10
--     ,REPLACE
...
CREATE TABLE Logging.LogEntry (
    ...
    ,Username sysname NOT NULL DEFAULT(CURRENT_USER)
...
BEGIN TRY
    -- BEGIN TRANSACTION
        ...
        BEGIN TRANSACTION
            ...
            RAISERROR(N'RowCount: %i',0,0, @@ROWCOUNT) WITH NOWAIT;
        COMMIT TRANSACTION
...
-- Max retries
DECLARE @_maxRetries tinyint = 5

-- retry count
DECLARE @_i tinyint = 0
...
BEGIN
    ...
    BEGIN
        IF @_isVerbose = 1
            RAISERROR(N'ProcessSqlExecRequestQueue:Turning on queue',0,0) WITH NOWAIT;
        ALTER QUEUE SqlExecProcessingQueue WITH STATUS = ON;
    END
    ELSE
    BEGIN
        IF @_isVerbose = 1
            RAISERROR(N'ProcessSqlExecRequestQueue:Queue is on',0,0) WITH NOWAIT;
    END

    IF @_isVerbose = 1
    BEGIN
        RAISERROR(N'ProcessSqlExecRequestQueue:Waiting for message...',0,0) WITH NOWAIT;
    END
    WAITFOR
    (
        RECEIVE TOP(1)
    ...
    IF @_isVerbose = 1
    BEGIN
        RAISERROR(N'ProcessSqlExecRequestQueue:Got message.',0,0) WITH NOWAIT;
    END
    ...
    BEGIN
        IF @_isVerbose = 1
        BEGIN
            RAISERROR(N'ProcessSqlExecRequestQueue:@conversationHandleId is null',0,0) WITH NOWAIT;
        END
        IF @@TRANCOUNT > 0
            COMMIT TRANSACTION
    ...
    BEGIN
        IF @_isVerbose = 1
        BEGIN
            RAISERROR(N'ProcessSqlExecRequestQueue:Timeout',0,0) WITH NOWAIT;
        END
        IF @@TRANCOUNT > 0
            ROLLBACK TRANSACTION;
    ...
    BEGIN
        IF @_isVerbose = 1
        BEGIN
            RAISERROR(N'ProcessSqlExecRequestQueue:SqlExecVoidRequest',0,0) WITH NOWAIT;
        END
        IF @@TRANCOUNT > 0
            COMMIT TRANSACTION
    ...
    BEGIN
        IF @_isVerbose = 1
        BEGIN
            RAISERROR(N'ProcessSqlExecRequestQueue:End dialog',0,0) WITH NOWAIT;
        END
        IF @@TRANCOUNT > 0
            ROLLBACK TRANSACTION
    ...
    BEGIN
        IF @_isVerbose = 1
        BEGIN
            RAISERROR(N'ProcessSqlExecRequestQueue:Error',0,0) WITH NOWAIT;
        END
        IF @@TRANCOUNT > 0
            ROLLBACK TRANSACTION
    ...
    BEGIN CATCH
        IF @_isVerbose = 1
        BEGIN
            RAISERROR(N'ProcessSqlExecRequestQueue:Exception',0,0) WITH NOWAIT;
        END
...
BEGIN
    DECLARE @request xml
    SET @request = dbo.GetAsyncRequestXml(@sql, @batchId)
